...
# ---------------------------------------------------------- Main Function ---------------------------------------------------------- #
def Download_Events(Settings: dict, Configuration: dict, window: CTk|None, Download_Date_Range_Source: str, Download_Data_Source: str, SP_Date_From_Method: str, SP_Date_To_Method: str, SP_Man_Date_To: str, SP_Password: str|None, Exchange_Password: str|None, Input_Start_Date: str|None, Input_End_Date: str|None) -> DataFrame:
    # Default
    ...
    File_Manipulation.Delete_File(Configuration=Configuration, window=window, file_path=Data_Functions.Absolute_path(relative_path=f"Operational\\Downloads\\Events_Registered.csv"))
        ...
        if Downloaded == True:
            ...
            if Events_Registered_df.empty:
                ...
                if My_Last_Day_dt >= Today:
                    ...
                    Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Last Day Reported is: {My_Last_Day_dt} --> cannot perform automatic Download. Use Manual.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
                else:
                    pass
            ...
            if SP_Date_From_Method == "Last Registered Date":
                ...
                Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Cannot define Start date as not know method selected. Method: {SP_Date_From_Method}", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
            ...
            if SP_Date_To_Method == "Today":
                ...
                if Input_End_Date_dt > Report_Period_End:
                    Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Question", message=f"Manual Date you entered {Input_End_Date_dt} is after Period End date {Report_Period_End}, will be automatically switched to Report End date. Do you agree?", icon="question",  option_1="Confirm", option_2="Reject", fade_in_duration=1, GUI_Level_ID=1)
                    if response == "Confirm":
                        Input_End_Date_dt = Report_Period_End
                    else:
                        Download_canceled = True
                        Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Download process canceled by user.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
                ...
                if Input_End_Date_dt >= Input_Start_Date_dt:
                    ...
                    Download_canceled = True
                    Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Download process canceled End date is before Start Date.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)

            elif SP_Date_To_Method == "Last Report Day":
                Input_End_Date_dt = Report_Period_End
            else:
                Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Cannot define End date as not know method selected. Method: {SP_Date_To_Method}", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
        else:
            Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"It was not possible to automatically download the master data from Sharepoint, please select Start Date and End Date manually.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)

        # Date check
        response = Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Question", message=f"Dates range to download: \n Date From: {Input_Start_Date_dt.strftime(Date_Format)}\n Date To: {Input_End_Date_dt.strftime(Date_Format)}", icon="question", option_1="Confirm", option_2="Reject", fade_in_duration=1, GUI_Level_ID=1)
        if response == "Confirm":
            pass
        else:
            Download_canceled = True
    ...
    # -------------- Manual  -------------- #
        ...
        Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Date source: {Download_Date_Range_Source} is not compatible, should be Sharepoint or Manual. Try again.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
    ...
    try:
        ...
        if Input_Start_Date_dt <= Input_End_Date_dt:
            ...
            Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Start Date is after End Date --> try again.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
        ...
        Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Something went wrong, try again.", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
    # Engine selection
    if Download_canceled == False:
        if Download_Data_Source == "Exchange":
            Events = Exchange.Download_Events(Settings=Settings, Configuration=Configuration, window=window, Input_Start_Date_dt=Input_Start_Date_dt, Input_End_Date_dt=Input_End_Date_dt, Filter_Start_Date=Filter_Start_Date, Filter_End_Date=Filter_End_Date, Exchange_Password=Exchange_Password) 
        else:
            Elements.Get_MessageBox(Configuration=Configuration, window=window, title="Error", message=f"Download source is not supported (Outlook_Client, API_Exchange_server), current is {Download_Data_Source}", icon="cancel", fade_in_duration=1, GUI_Level_ID=1)
    else:
        pass
